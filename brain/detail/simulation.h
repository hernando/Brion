...
char const* const _defaultSonataSpikeFile = "/spikes.h5";
}
...
public:
    ...
    Strings getTargetNames() const final { return Strings(); }
    GIDSet getGIDs() const final { return GIDSet(); }
    GIDSet getGIDs(const std::string&) const { return GIDSet(); }
    Strings getCompartmentReportNames() const final
    {
    ...
    brion::URI getSpikeReportSource() const final
    {
        return brion::URI(_config.getOutputRoot() + _defaultSonataSpikeFile);
    }

    brion::URI getCompartmentReportSource(const std::string& name) const final
    {
        // We don't check if the report exist.
        return brion::URI(_config.getOutputRoot() + "/" + name + ".h5");
    }
