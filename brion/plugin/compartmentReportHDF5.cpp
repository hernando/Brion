...
constexpr auto _cacheSizeStr = "cache_size";
constexpr auto _chunkSizeStr = "chunk_size";
...
{
    const auto keyValueIter = uri.findQuery(_cacheSizeStr);
    ...
    return _parseSizeOption(value, _cacheSizeStr);
}
...
void CompartmentReportHDF5::_allocateDataSet()
{
    ...
    caching.add(
        HighFive::Caching(chunksPerFrame * _chunkSize, chunksPerFrame + 1));
...
void CompartmentReportHDF5::_parseWriteOptions(const URI& uri)
{
    for (auto i = uri.queryBegin(); i != uri.queryEnd(); ++i)
    {
        ...
        if (key == _chunkSizeStr)
        {
            const size_t chunkSize = _parseSizeOption(value, _chunkSizeStr);
            if (chunkSize != 0)
